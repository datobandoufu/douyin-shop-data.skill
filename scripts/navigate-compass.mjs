...
async function main() {
  ...
  // 在浏览器中新建一个标签页并直接打开电商罗盘
  const url = 'https://compass.jinritemai.com/shop';
  ...
  // 附加到该标签页会话，读取当前 URL / title 确认已进罗盘
  ...
  const ok = cur.url.includes('compass.jinritemai.com');
  console.log(ok ? '[OK] 已进入电商罗盘' : '[WARN] 未确认进入电商罗盘，请检查登录态');
